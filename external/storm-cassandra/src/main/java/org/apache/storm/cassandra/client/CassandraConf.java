...
public class CassandraConf implements Serializable {
    ...
    public CassandraConf(Map<String, Object> conf) {
    	...
    	if (scyllaParams != null) {
            ...
            this.username = scyllaParams.getUser();
	        this.password = scyllaParams.getPassword();
	        this.nodes    = scyllaParams.getNodesHostList().split(","); 
	        this.port     = Integer.valueOf(scyllaParams.getNodesPort());
            this.sslSecurityProtocol = scyllaParams.getSslParams().getSecurityProtocol();
            this.sslKeystorePath     = scyllaParams.getSslParams().getSslKeystorePath();
            this.sslKeystorePassword = scyllaParams.getSslParams().getSslKeystorePassword();
            this.sslTruststorePath   = scyllaParams.getSslParams().getSslTruststorePath();
            this.sslTruststorePassword =scyllaParams.getSslParams().getSslTruststorePassword();
            this.consistencyLevel = ConsistencyLevel.valueOf(scyllaParams.getConsistencyLevel());
    	} else {
	        ...
	        this.consistencyLevel = ConsistencyLevel.valueOf((String) Utils.get(conf, CASSANDRA_CONSISTENCY_LEVEL, ConsistencyLevel.LOCAL_QUORUM.name()));
